package com.aoro.gimnasio.utils;

public class Constants {

	public static final String URL_LOGIN = "/login";
	///// catproductos
	public static final String CAT_PRODUCTOS_GET_ALL = "/catproductos/getAll";
	public static final String CAT_PRODUCTOS_SAVE_UPDATE = "/catproductos/saveUpdate";
	public static final String CAT_PRODUCTOS_DELETE = "/catproductos/delete";
	////// membresias
	public static final String MEMBRESIAS_GET_ALL = "/membresias/getAll";
	public static final String MEMBRESIAS_SAVE_UPDATE = "/membresias/saveUpdate";
	public static final String MEMBRESIAS_DELETE = "/membresias/delete";
	////// socios
	public static final String SOCIOS_GET_ALL = "/socios/getAll";
	public static final String SOCIOS_SAVE_UPDATE = "/socios/saveUpdate";
	public static final String SOCIOS_DELETE = "/socios/delete";
	////// usuarios
	public static final String USUARIOS_GET_ALL = "/usuarios/getAll";
	public static final String USUARIOS_SAVE_UPDATE = "/usuarios/saveUpdate";
	public static final String USUARIOS_DELETE = "/usuarios/delete";
	public static final String USUARIOS_SOCIOS_GET_ALL = "/usuarios/getUserParaSocio";
	////// ventas
	public static final String VENTA_GET_DIARIA = "/venta/ventaDiaria";
	public static final String VENTA_GET_DETALLE_DIARIA = "/venta/detalleVtaDiaria";

}
